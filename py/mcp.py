...
def _TOOLS_TEMPLATE(NOTES_FORMAT_GUIDE, LANG_HINT):
    ...
    },
    {
        ...
        "description": (
            ...
            "\n"
            "Live plants (potted, bare-root, cuttings): if the user hasn't said whether "
            "the plant is being installed now or kept aside, ask ONCE. Then act without "
            "further confirmation:\n"
            "  • Installed now (the default for most live-plant purchases) → this is "
            "ALWAYS TWO EVENTS, created back-to-back in the same turn:\n"
            "      1) add_plant with first_event='acquire', price, source, "
            "acquire_type='bought', event_date=purchase date.\n"
            "      2) IMMEDIATELY follow up in the SAME turn with log_event for "
            "event_type='plant', event_date=same purchase date, plant_id=the id returned "
            "by step 1. Do NOT ask the user whether to log the plant event; do NOT stop "
            "after the acquire. The user already told you it's installed; that means "
            "both events. Skipping step 2 is a bug.\n"
            "      The result is the correct timeline order: 'Acheté chez …' first, "
            "then 'Plantation' on the same day (events with the same date sort by "
            "insertion order).\n"
            "      NEVER use first_event='plant' instead — the price has nowhere to "
            "live and the purchase context is lost.\n"
            "  • Kept in stash → ONE event: first_event='acquire' (with price/source). "
            "No follow-up.\n"
            "  • Pre-ordered, not yet received → ONE event: first_event='order' (with "
            "price, source, expected_date). When it arrives later, the user marks it "
